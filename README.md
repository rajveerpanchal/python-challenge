...
## if using terminal cmd :

like dictionary written In Windows Command Prompt (cmd), replace single quotes (') with escaped double quotes (\")

=> restful.bat post /posts -d "{\"title\": \"New Post\", \"body\": \"Testing API\", \"userId\": 2}"

## if using terminal powershell :

like dictionary data written In PowerShell, use single quotes (') for the entire argument, but ensure JSON keys and values are double-quoted inside:

=> restful.bat post /posts -d '{\"title\": \"Hello World\", \"body\": \"This is a test post.\", \"userId\": 1}'
